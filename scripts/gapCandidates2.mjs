...
const CANDIDATES = [
  // 1回目でid衝突・norm()の類似名判定に引っかかったもの。名前とidを実態に合わせて再挑戦。
  ['chichibu-onsen', '秩父温泉満願の湯', '埼玉県', '皆野町', ['秩父温泉'],   ['満願の湯']],
  ['obama-nagasaki', '小浜温泉（雲仙）', '長崎県', '雲仙市', ['小浜温泉'],   ['小浜温泉']],
  ['kanba-taki',     '神庭の滝',        '岡山県', '真庭市', ['神庭の滝'],   ['神庭の滝']],
];
...
fs.writeFileSync('logs/gap_candidates2.json', JSON.stringify(out, null, 1));
console.log(`\n合格 ${passed.length} / ${out.length} 件 → logs/gap_candidates2.json`);
