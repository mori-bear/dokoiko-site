#!/usr/bin/env node
/**
 * machinamiCandidates2.mjs — 1回目で「2ソース未満」だった9地区の再挑戦。
 * 落ちた原因は地区名がWikipedia/Wikidataの項目名と合っていなかったこと。
 * 記事名候補とOSMトークンを実際の呼称に寄せて引き直す。
 * （元の説明）machinamiCandidates.mjs — 重要伝統的建造物群保存地区129件のうち、既存destinationsに
...
const CANDIDATES = [
  ['murata-miyagi',  '村田の蔵の町',   '宮城県',   '村田町',     ['村田町 (宮城県)', '村田町'],        ['村田町蔵の町並み', '村田町']],
  ['kaemon',         '嘉右衛門町',     '栃木県',   '栃木市',     ['嘉右衛門町', '栃木市'],            ['嘉右衛門町', '栃木市嘉右衛門町']],
  ['akaiwa-gunma',   '六合赤岩',       '群馬県',   '中之条町',   ['赤岩 (群馬県)', '六合村', '中之条町'], ['赤岩集落', '赤岩']],
  ['shioyama-kamijo','塩山下小田原上条', '山梨県',   '甲州市',     ['塩山下小田原上条', '甲州市'],      ['下小田原上条', '上条集落']],
  ['inariyama',      '稲荷山宿',       '長野県',   '千曲市',     ['稲荷山宿', '稲荷山 (長野県)', '千曲市'], ['稲荷山宿', '稲荷山']],
  ['oya-osugi',      '大屋町大杉',     '兵庫県',   '養父市',     ['大杉 (兵庫県)', '養父市'],         ['大杉集落', '大屋町大杉']],
  ['doikachu',       '土居廓中',       '高知県',   '安芸市',     ['土居廓中', '安芸市'],              ['土居廓中', '安芸城']],
  ['irikifumoto',    '入来麓',         '鹿児島県', '薩摩川内市', ['入来麓武家屋敷群', '入来麓', '入来町'], ['入来麓武家屋敷群', '入来麓']],
  ['kaseda-fumoto',  '加世田麓',       '鹿児島県', '南さつま市', ['加世田麓', '加世田', '南さつま市'],  ['加世田麓', '加世田']],
];
...
fs.writeFileSync('logs/machinami_candidates2.json', JSON.stringify(out, null, 2));
console.log(`\n合格 ${passed.length} / ${out.length} 件 → logs/machinami_candidates2.json`);
