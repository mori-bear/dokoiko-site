...
/**
 * refetchOverseas.js
 * logs/vision_overseas.json の287件を Wikimedia Commons のみで再取得し、
 * Vision で「日本国内・destinationとして適切」を再検証してから置換する。
 * Unsplashは一切使用しない。最大3候補、全滅なら既存維持＋スキップ記録。
 * 結果: logs/overseas_refetch.json（逐次保存・再開可）
 */
import fs from 'fs';
...
const OUT = 'logs/overseas_refetch.json';
...
const targets = JSON.parse(fs.readFileSync('logs/vision_overseas.json', 'utf8'));
