#!/usr/bin/env node
/** qaMachinami.mjs — 新ジャンル「街並み」（重伝建）の機械チェック。観点は温泉版と同じ。 */
import fs from 'fs';
...
const PREF_BOX = {
  ...
  '石川県': [36.0, 37.6, 136.2, 137.4], '和歌山県': [33.4, 34.4, 135.0, 136.1], '福井県': [35.3, 36.3, 135.4, 136.9],
...
console.log(`■ 街並み（重伝建）新規${NEW.length}件のQA`);
for (const id of NEW) {
  const x = byId[id];
