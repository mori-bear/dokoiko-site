#!/usr/bin/env node
/** hitouTravelTimeBase2.mjs — 第2バッチ13件の travelTime/icCard の土台にする最寄り既存エントリを出す。 */
import fs from 'fs';
...
const IDS = ['horoka', 'osawa-onsen', 'yubama', 'kanigasaki', 'utto', 'seorasou', 'yunohana-fk',
  'tsubame-onsen', 'yumata', 'kamikitayama', 'iwai-tottori', 'misasa2', 'yuki-hiroshima'];
const targets = JSON.parse(fs.readFileSync('logs/hitou_targets2.json', 'utf8')).filter((t) => IDS.includes(t.id));
for (const t of targets) {
  const near = all
    .filter((d) => typeof d.lat === 'number' && d.travelTime && Object.keys(d.travelTime).length >= 5)
    .map((d) => ({ id: d.id, name: d.name, pref: d.prefecture, km: kmBetween(t.lat, t.lng, d.lat, d.lng), n: Object.keys(d.travelTime).length, ic: d.icCard }))
