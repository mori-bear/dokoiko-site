#!/usr/bin/env node
/** qaKeikan.mjs — 新ジャンル「絶景」（国指定名勝）の機械チェック。観点は温泉版と同じ。 */
import fs from 'fs';

const NEW = ['fukiware', 'myogisan', 'sasagawa', 'shomyodaki', 'sotomo', 'rurikei',
...
const PREF_BOX = {
  ...
  '石川県': [36.0, 37.6, 136.2, 137.4], '京都府': [34.7, 35.8, 134.8, 136.1],
...
console.log(`■ 絶景（国指定名勝）新規${NEW.length}件のQA`);
for (const id of NEW) {
  const x = byId[id];
